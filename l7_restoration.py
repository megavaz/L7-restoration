...
def create_train_data(target_image, training_images, target_mask, train_mask, j,
                      window_size, geospatial=True):
    ...
    train_data = np.array([training_images_window[m][train_mask_window] for m in
                           range(len(training_images_window))])
    ...
    restore_data = np.array(
        [training_images_window[m][target_mask_window] for m in
         range(len(training_images_window))])
    ...
    return train_data, train_label, restore_data, \
           target_mask_window, target_image_window


def create_train_data_quick(target_image, training_images, target_mask,
                            train_mask, i, j, window_size, geospatial=True):
    ...
    training_images_window = training_images[:, i: i + window_size,
                             j: j + window_size]
    train_data = np.array([training_images_window[m][train_mask_window] for m in
                           range(len(training_images_window))])
    train_label = target_image_window[train_mask_window]
...
def create_order(img):
    number_of_skips = [np.count_nonzero(img[i] == 0) for i in
                       range(img.shape[0])]
    order = [i for i in range(img.shape[0])]
    order = [x for x, _ in
             sorted(zip(order, number_of_skips), key=lambda pair: pair[1])]
    return order
...
def create_horizontal_iteration(img, mask, average_segment_length=1):
    ...
    horizontal_iteration = horizontal_iteration + [i for i in
                                                   range(coordinates,
                                                         img.shape[-1],
...
def _dummy_restore(img_volume, mask_volume):
    ...
    for channel in range(img_volume.shape[3]):
        ...
        if np.count_nonzero(mask) != 0:
            ...
            for number, i, j in zip(numbers.tolist(), i_s.tolist(),
                                    j_s.tolist()):
                img_volume[number, i, j, channel] = img_volume[
                    number, i - 1, j, channel] if i - 1 > 0 else img_volume[
...
def _quick_restore(
    ...
    for target_image_number in tqdm(restoration_order, disable=not verbose):
        ...
        training_images = np.concatenate([img_volume[:target_image_number],
                                          img_volume[target_image_number + 1:]],
                                         axis=0)
        threshold = (target_image.shape[0] - window_size,
                     target_image.shape[1] - window_size)
        for i in range(0, target_image.shape[0], window_size):
            for j in range(0, target_image.shape[1], window_size):
                ...
                for channel in range(img_volume.shape[-1]):
                    ...
                    target_mask_window, \
                    target_image_window = create_train_data_quick(
                    ...
                    try:
                        ...
                        res = np.full((restore_data.shape[0],),
                                      np.average(train_label[0]))
                    target_image_window[target_mask_window] = res
                    img_volume[target_image_number, i:i + window_size,
                    j: j + window_size,
...
def _restore_images(
    ...
    for target_image_number in tqdm(restoration_order, disable=not verbose):
        ...
        training_images = np.concatenate([img_volume[:target_image_number],
                                          img_volume[target_image_number + 1:]],
                                         axis=0)
        for train_mask_current, target_mask_current in zip(train_masks,
                                                           target_masks):
                ...
                for channel in range(img_volume.shape[-1]):
                    ...
                    try:
                        ...
                        res = np.full((restore_data.shape[0],),
                                      np.average(train_label[0]))
                    target_image_window[target_mask_window] = res
                    img_volume[target_image_number, :, j: j + window_size,
                    channel] = target_image_window
    img_volume = np.nan_to_num(img_volume, nan=0.0)
    img_volume = np.clip(img_volume, min_value, max_value)
    return img_volume
...
def quick_restore(
    ...
    try:
        assert check_intersection(~mask_volume) == 0
    except AssertionError:
        print(
            'Your images do not fully cover all missing pixels! Add more '
            'images to your batch.')
...
def main():
    ...
    parser.add_argument('path_to_file', type=str,
                        help='Path to the file you want to restore. Currently '
                             'supported '
                             'formats are .npz and .tif. Note that your file '
                             'should have '
                             'shape (num_images, height, width, num_channels).')
    parser.add_argument('path_to_mask', type=str,
                        help='Path to the mask. Note that the mask must be of'
                             'shape (num_images, height, width). You can '
                             'create proper '
                             'mask with "create_mask.py".')
    parser.add_argument('--algorithm', type=str, default='quick',
                        choices=['quick', 'full'],
                        help='Type of the algorithm to use.')
    parser.add_argument('--silent', action='store_true')
    parser.add_argument('--order', type=int, action='extend', nargs='+',
                        help='You can specify which images you want to '
                             'restore and the order of restoration. '
                             "If you don't specify this parameter, all of "
                             "your images will be restored."
                             "Note that if you use full version of the "
                             "algorithm even if you specify only "
                             "part of the images to be restored, still all of "
                             "your images will be restored"
                             "but the full version of the algorithm will be "
                             "applied only to specified images."
                             "If you don't" 'want this behavior use option '
                             '"--noquick"')
    ...
    print('Restoration has ended! Result is saved in file "res.{}"'.format(
        extension))
